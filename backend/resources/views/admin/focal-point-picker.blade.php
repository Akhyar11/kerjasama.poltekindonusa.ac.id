@php
    $record = $getRecord();
    $coverImage = $getContainer()->getRawState()['cover_image'] ?? ($record?->cover_image ?? null);
    ...
    $imageUrl = null;
    if ($coverImage) {
        if (is_object($coverImage) && method_exists($coverImage, 'temporaryUrl')) {
            try {
            ...
            $imageUrl = str_starts_with($coverImage, 'http') ? $coverImage : asset('storage/' . $coverImage);
        }
    }
@endphp
...
<div class="filament-focal-point-picker-wrapper" style="margin-top: 10px; margin-bottom: 10px;">
    <div style="font-size: 14px; font-weight: 500; margin-bottom: 8px; color: #111827;" class="dark:text-white">
        Fokus Gambar Cover (Klik/Geser untuk Mengatur)
    </div>
    ...
    <div
        ...
        x-ref="container"
        style="position: relative; width: 100%; height: 280px; overflow: hidden; border: 1.5px solid #d1d5db; border-radius: 8px; background-color: #f3f4f6; cursor: crosshair; box-sizing: border-box;"
    ...
    >
        ...
        @else
            <div style="position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; background: linear-gradient(135deg, #e5e7eb, #d1d5db); opacity: 0.8; padding: 16px; text-align: center; pointer-events: none;" class="dark:from-gray-800 dark:to-gray-900">
                ...
                <span style="font-size: 13px; color: #4b5563; font-weight: 500;" class="dark:text-gray-400">Unggah gambar cover terlebih dahulu untuk melihat pratinjau fokus</span>
            </div>
        @endif
        ...
        <div style="position: absolute; bottom: 8px; left: 8px; padding: 6px 12px; background-color: rgba(17, 24, 39, 0.85); color: #ffffff; font-size: 12px; font-weight: 600; border-radius: 6px; pointer-events: none; z-index: 20; box-shadow: 0 2px 6px rgba(0,0,0,0.25);">
            Fokus: <span x-text="state">50% 50%</span> (Klik / Seret di area gambar)
        </div>
    </div>
</div>
